      ...
      <form onSubmit={handleSubmit} className="flex-1 max-w-lg">
        <div className="relative">
          ...
          <input
            ...
            className="block w-full pl-10 pr-24 py-2 border border-slate-300 rounded-lg text-sm bg-slate-50 placeholder-slate-400 focus:outline-none focus:bg-white focus:ring-2 focus:ring-orange-500/20 focus:border-semrush-orange transition-all"
          />
          <div className="absolute inset-y-1.5 right-1.5">
            <button
              type="submit"
              disabled={isLoading}
              className="px-4 py-1 bg-semrush-orange hover:bg-semrush-orange/95 text-white font-medium text-xs rounded-md shadow-sm transition-colors flex items-center gap-1.5 h-full"
            >
              {isLoading ? (
      ...
      <div className="flex items-center gap-6 ml-4">
        {currentDomain && (
          <div className="hidden md:flex items-center gap-2 px-3 py-1.5 bg-slate-100 border border-slate-200 rounded-full text-xs font-semibold text-slate-700">
            ...
            <span>Analyzing: <span className="text-semrush-orange font-bold">{currentDomain}</span></span>
          </div>
        )}
        ...
        <div className="flex items-center gap-3">
          <button className="p-2 text-slate-400 hover:text-slate-600 rounded-full hover:bg-slate-50 transition-colors relative">
            ...
            <span className="absolute top-1.5 right-1.5 w-2 h-2 bg-semrush-danger rounded-full border border-white"></span>
          </button>
